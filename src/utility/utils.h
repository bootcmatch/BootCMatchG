...
namespace Parallel{
  template <typename T>
  T* max(T *a, int n, bool host_result=false, cudaStream_t stream=DEFAULT_STREAM);

  template <typename T>
  T* cumsum(T *a, int n, bool host_result=false, cudaStream_t stream=DEFAULT_STREAM);
}
