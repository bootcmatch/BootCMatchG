...
#define NTHREAD_PER_BLK 128 // numero di thread per blocco; numero di warp per blocco = NTHREAD_PER_BLK / 32
#define CHUNK_PER_WARP 16 // numero di vertici assegnati a singolo WARP
#define WARP_SIZE 32
...
__device__ void set_suitor(int candidate, double heaviest, int current_vid, int *finished,int *new_vertex_to_care,  volatile double*  ws_, volatile int*  s_, volatile int*  d_locks) {
    ...
    *finished = TRUE;
    ...
    if (heaviest > 0 && candidate >= 0) {
        ...
        *finished = FALSE; /* case when you don't get lock */
            ...
            if (lock_vertex(d_locks, candidate)) { // lock "candidate"
                ...
                if (heaviest >= ws_[candidate]) { // test whether "heaviest" is still larger than previous offer to "candidate"
                    ...
                    *finished = TRUE;
                    ...
                    //*partner = candidate; // information to return
                    //*weight_with_partner = heaviest; // information to return
                    ...
                    if (next_vertex >= 0) { // take responsibility of un-lodged vertex. Find suitor for it.
                        ...
                        //done = FALSE; //  not yet done, need to Find suitor for un-lodged one
                        *finished = FALSE;
                    }
...
__device__ void find_Candidate(int W_OFF, int len_neighborlist, int *edge_list, volatile double*  ws_, volatile int*  s_, double *weight_list, volatile int*  d_locks, int current_vid, int *potential_candidate,double *by_weight) {
    ...
    for (int i = WARP_SIZE / 2; i >= 1; i /= 2) {
        ...
        double rec_value = __shfl_xor_sync(mask, heaviest, i, WARP_SIZE); //double

        int rec_vertex = __shfl_xor_sync(mask, candidate, i, WARP_SIZE); // integer
...
__device__ void collectUnsucc(int laneId,  volatile int*  warpmem, int *nr_of_unsuccess, int isfinished, int new_vertex_to_care) {
    ...
    for (int i = 1; i <= WARP_SIZE / 2; i *= 2) {
        ...
        int lowerLaneValue = __shfl_up_sync(mask, value, i, WARP_SIZE);
        //if ( (threadIdx.x & 0x1f) >= i)
        if (laneId >= i)
    ...
    *nr_of_unsuccess += __shfl_sync(mask, value, WARP_SIZE - 1, WARP_SIZE);

}



__global__ void kernel_for_matching(  int n, int *d_indices, volatile int*  suitors, volatile double*  ws, int *d_edges, double *d_weights, volatile int*  d_locks) {
    ...
    //if(!laneId )
    	//printf(" %d: [%d %d) \n", wid+ blockIdx.x*(NTHREAD_PER_BLK/warpSize), (blockIdx.x*(NTHREAD_PER_BLK/warpSize)+wid)*CHUNK_PER_WARP, (blockIdx.x*(NTHREAD_PER_BLK/warpSize)+wid+1)*CHUNK_PER_WARP );
    ...
    for (int vid = nr_element_to_read-1; vid >=0; vid--) {
    	...
    	//for (int vid =  wid*CHUNK_PER_WARP  ; vid< n && vid < (wid+1)*CHUNK_PER_WARP  ; vid++) {

    	//if(!laneId)
    		//printf("%d %d\n",wid,vid);

    	/*// related to for loop with actual vertex id
    	neighbor_list= &d_edges[ warpmem[vid-wid*CHUNK_PER_WARP] ];  //&d_edges[d_indices[vid]];
    	related_weights=&d_weights[ warpmem[vid-wid*CHUNK_PER_WARP] ]; //&d_weights[d_indices[vid]];

    	num_neighbors=warpmem[vid-wid*CHUNK_PER_WARP+1]- warpmem[vid-wid*CHUNK_PER_WARP]; //d_indices[vid+1]-d_indices[vid];
    	*/
    	...
    	//find_Candidate(int W_OFF, int len_neighborlist, int *edge_list, double *ws_,int *s_, double *weight_list, int *d_locks, int current_vid, int *potential_candidate,double *by_weight)
    	find_Candidate(laneId,num_neighbors,neighbor_list,ws,suitors,related_weights,d_locks,vid+wid*CHUNK_PER_WARP ,&candidate,&by_weight);
    	//if(!laneId)
    		//printf("%d-->%d\n",vid,candidate);
    	//if(!laneId)
    		//printf("%d %d\n",v,candidate);
    	...
    	if( (acc%WARP_SIZE)== laneId ){
    		...
    		//saved_vertex=vid; // save vertex , related to for loop with actual vertex id
    	...
    	if( (acc%WARP_SIZE)==0){
    		//isfinished=TRUE;
    		//set_suitor(int candidate, double heaviest, int current_vid, int *finished,int *new_vertex_to_care, double *ws_, int *s_, int *d_locks)
    ...
    do{
    	...
    	*/
    	...
    	for (int k = 0; k < prev_nr_of_unsuccess; ++k) {
    		...
    		by_weight=-5.0;
    		//find_Candidate(int W_OFF, int len_neighborlist, int *edge_list, double *ws_,int *s_, double *weight_list, int *d_locks, int current_vid, int *potential_candidate,double *by_weight)
    		find_Candidate(laneId,num_neighbors,neighbor_list,ws,suitors,related_weights,d_locks,new_vertex,&candidate,&by_weight);
...
double cost_matching(int tot_vertices, int64_t tot_edges, int n, int *ver, int *edges, double *weight, int *match) {
    ...
    //printMetaData("matching;size", (int)tot_card, 0);
    //printMetaData("matching;sum_w", glob_sum, 1);

    printf("Total Match=%ld where #Vertex: %d #Edge: %ld  \n", tot_card, tot_vertices,tot_edges);
    return (glob_sum);
}

vector<int>* approx_match_gpu_suitor(CSR *W, vector<itype> *M, vector<double> *ws, vector<int> *mutex) {

    assert(W->on_the_device);

    int n = W->n;

    //vector<int> *M = Vector::init<int>(n, true, true); // marked
    Vector::fillWithValue(M, -1);
    //vector<double> *ws = Vector::init<double>(n, true, true); // ws
    Vector::fillWithValue(ws, 0.0);
    //vector<int> *mutex = Vector::init<int>(n, true, true); // locks
    ...
    kernel_for_matching<<<nr_of_block, NTHREAD_PER_BLK, shared_memory_size_per_block>>>(n, W->row, M->val, ws->val, W->col, W->val, mutex->val);
...
#define EPS 3.885780586188048e-10
#define MY_GT(a, b) (a - b) > EPS
  ...
  for(int i=0; i<n; i++){
    ...
    while(!done){
      ...
      for(int j=row[current]; j<row[current+1]; j++){
        int v = col[j];
        if(MY_GT(val[j], heaviest) && MY_GT(val[j], ws[v])){
          partner = v;
          heaviest = val[j];
        }
      }
